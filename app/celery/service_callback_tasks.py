...
def send_delivery_status_to_service(
    ...
    try:
        ...
        current_app.logger.error(
            "Not retrying: %s failed for %s, url: %s. ",
            self.name,
            logging_tags,
            service_callback.url,
        )
...
@notify_celery.task(
    bind=True, name="send-complaint-to-vanotify", max_retries=5, default_retry_delay=300
)
@statsd(namespace="tasks")
def send_complaint_to_vanotify(
    self, complaint_id: str, complaint_template_name: str
) -> None:
...
def send_inbound_sms_to_service(self, inbound_sms_id, service_id):
    service_callback = get_service_inbound_sms_callback_api_for_service(
...
def create_delivery_status_callback_data(
    notification, service_callback_api, provider_payload=None
):
...
def _check_and_queue_complaint_callback_task(complaint, notification, recipient):
    ...
    if service_callback_api:
        ...
        send_complaint_to_service.apply_async(
            [service_callback_api.id, complaint_data], queue=QueueNames.CALLBACKS
        )


def publish_complaint(
    complaint: Complaint, notification: Notification, recipient_email: str
) -> bool:
    ...
    send_complaint_to_vanotify.apply_async(
        [str(complaint.id), notification.template.name], queue=QueueNames.NOTIFY
    )
