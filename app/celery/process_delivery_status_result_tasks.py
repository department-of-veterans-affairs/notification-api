...
def process_delivery_status(self, event: CeleryEvent) -> bool:
    ...
    notification_platform_status = _get_notification_platform_status(
        self, provider, body, sqs_message
    )
...
def check_notification_status(
    notification: Notification, notification_status: str
) -> bool:
...
def _get_notification_parameters(
    notification_platform_status: dict,
    ...
    number_of_message_parts = notification_platform_status.get(
        "number_of_message_parts", 1
    )
    price_in_millicents_usd = notification_platform_status.get(
...
def _get_notification_platform_status(
    self, provider: any, body: str, sqs_message: dict
) -> dict:
...
def _get_include_payload_status(self, notification: Notification) -> bool:
    ...
    try:
        ...
        current_app.logger.info(
            "Payload should be included: %s", include_payload_status
        )

    except (AttributeError, TypeError) as e:
        current_app.logger.error(
            "Could not determine include_payload property for ServiceCallback."
        )
        current_app.logger.exception(e)
...
def _increment_statsd(
    notification: Notification, provider_name: str, notification_status: str
) -> None:
