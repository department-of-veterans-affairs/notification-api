...
def _delete_for_query(subquery):
    number_deleted = (
        db.session.query(Notification).filter(Notification.id.in_(subquery)).delete(synchronize_session='fetch')
    )
...
def dao_get_scheduled_notifications():
    notifications = (
        ...
        .filter(ScheduledNotification.scheduled_for < datetime.utcnow(), ScheduledNotification.pending)
        .all()
    )
