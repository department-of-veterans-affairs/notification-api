...
class VAOnsiteClient:
    ...
    def post_onsite_notification(self, data: dict):
        ...
        self.logger.info(f"Sending this data with POST request to onsite_notifications: %s", data)
        ...
        try:
            ...
            self.logger.info("onsite_notifications POST response: status_code=%i, json=%s", response.status_code, response.json())
