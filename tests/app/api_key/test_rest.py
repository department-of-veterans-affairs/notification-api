...
def test_get_api_keys_ranked(notify_db_session, admin_request, sample_api_key, sample_template):
    template = sample_template(template_type=EMAIL_TYPE)
    ...
    for _ in range(total_sends):
        ...
        notifications.append(create_notification(template=template, api_key=key_1))
