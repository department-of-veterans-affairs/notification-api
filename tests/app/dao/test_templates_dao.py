...
def test_create_only_one_template(
    ...
    templates = notify_db_session.session.scalars(select(Template)).all()
...
def test_get_all_templates_ignores_hidden_templates(
    ...
    hidden_template = create_template(
        template_name=str(uuid4()),
        hidden=True,
        service=service
    )
...
def test_get_template_by_id_and_service(
    ...
    template_0 = create_template(
        template_name=original_name,
        service=service)
    template_1 = dao_get_template_by_id_and_service_id(
        template_id=template_0.id,
        service_id=service.id)
