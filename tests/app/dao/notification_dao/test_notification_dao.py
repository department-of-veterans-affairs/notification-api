...
def test_should_not_update_status_by_reference_if_from_country_with_no_delivery_receipts(sample_template):
    notification = create_notification(sample_template, status=NOTIFICATION_DELIVERED, reference='foo')
...
def test_should_not_update_status_once_notification_status_is_delivered(sample_email_template):
...
def test_get_notification_for_job(sample_notification):
    notification_from_db = get_notification_for_job(
        sample_notification.service.id, sample_notification.job_id, sample_notification.id
    )
    assert sample_notification == notification_from_db
...
def test_should_delete_recipient_identifiers_if_notification_deleted(notify_db, sample_job, sample_api_key, mocker):
...
def test_get_notifications_created_by_api_or_csv_are_returned_correctly_excluding_test_key_notifications(
    notify_db, notify_db_session, sample_service, sample_job, sample_api_key, sample_team_api_key, sample_test_api_key
):
...
def test_dao_get_notifications_by_to_field_only_searches_one_notification_type(
    ...
    email = create_notification(template=email_template, to_field='165@example.com', normalised_to='165@example.com')
...
def test_dao_created_scheduled_notification(sample_notification):
    scheduled_notification = ScheduledNotification(
        notification_id=sample_notification.id, scheduled_for=datetime.strptime('2017-01-05 14:15', '%Y-%m-%d %H:%M')
    )
...
def test_dao_get_notifications_by_to_field_filters_status(sample_template):
    notification = create_notification(
        template=sample_template, to_field='+16502532222', normalised_to='+16502532222', status='delivered'
    )
    create_notification(
        template=sample_template, to_field='+16502532222', normalised_to='+16502532222', status='temporary-failure'
    )
...
def test_dao_get_notifications_by_to_field_filters_multiple_statuses(sample_template):
    ...
    notification2 = create_notification(
        template=sample_template, to_field='+16502532222', normalised_to='+16502532222', status='sending'
    )
...
def test_dao_get_notification_history_by_reference_with_one_match_returns_notification(sample_letter_template):
    create_notification(template=sample_letter_template, reference='REF1')
...
def test_dao_get_notification_history_by_reference_with_multiple_matches_raises_error(sample_letter_template):
    create_notification(template=sample_letter_template, reference='REF1')
    create_notification(template=sample_letter_template, reference='REF1')
...
def test_update_notification_status_by_id_cannot_exit(sample_template, next_status):
    reference = str(uuid.uuid4())

    # create notification object
    create_notification(sample_template, reference=reference, sent_at=datetime.now())
...
def test_dao_update_notification_will_update_last_updated_without_conditions(sample_template):
    ...
    # create notification object
    create_notification(sample_template, reference=reference, sent_at=datetime.now(), status=NOTIFICATION_DELIVERED)
...
def test_update_notification_status_by_id_cannot_update_status_out_of_order_with_invalid_values(
    sample_template, current_status, next_status
):
    reference = str(uuid.uuid4())

    # create notification object
    create_notification(sample_template, reference=reference, sent_at=datetime.now(), status=current_status)
...
def test_update_notification_status_by_id_can_update_status_in_order_when_given_valid_values(
    ...
    for ref in reference_tuple:
        ...
        create_notification(sample_template, reference=ref, sent_at=datetime.now(), status=current_status)
