...
def setup_provider_details(notify_db_session):
    notify_db_session.session.query(ProviderRates).delete()
    notify_db_session.session.query(ProviderDetails).delete()
    ...
    notify_db_session.session.add(prioritised_email_provider)
    ...
    notify_db_session.session.add(deprioritised_email_provider)
    ...
    notify_db_session.session.add(prioritised_sms_provider)
    ...
    notify_db_session.session.add(deprioritised_sms_provider)
    ...
    notify_db_session.session.add(inactive_sms_provider)
...
@pytest.fixture(scope='function')
def setup_sms_providers(notify_db_session):
    notify_db_session.session.query(ProviderRates).delete()
    notify_db_session.session.query(ProviderDetails).delete()
    notify_db_session.session.query(ProviderDetailsHistory).delete()
    ...
    notify_db_session.session.add_all(providers)
    return providers


@pytest.fixture(scope='function')
def setup_sms_providers_with_history(notify_db_session, setup_sms_providers):
    notify_db_session.session.query(ProviderDetailsHistory).delete()
    providers_history = [ProviderDetailsHistory.from_original(provider) for provider in setup_sms_providers]
    notify_db_session.session.add_all(providers_history)
    return setup_sms_providers


@pytest.fixture(scope='function')
def setup_equal_priority_sms_providers(notify_db_session):
    notify_db_session.session.query(ProviderRates).delete()
    notify_db_session.session.query(ProviderDetails).delete()
...
def commit_to_db(notify_db_session, *providers):
    notify_db_session.session.query(ProviderRates).delete()
    notify_db_session.session.query(ProviderDetails).delete()
...
class TestGetHighestPriorityActiveProviderByNotificationType:
    ...
    def test_gets_matching_type(self, notify_db_session):
        ...
        commit_to_db(notify_db_session, email_provider, sms_provider)
    ...
    def test_gets_higher_priority(self, notify_db_session):
        ...
        commit_to_db(notify_db_session, low_number_priority_provider, high_number_priority_provider)
    ...
    def test_gets_active(self, notify_db_session):
        active_provider = self.provider_factory(active=True)
        inactive_provider = self.provider_factory(active=False)

        commit_to_db(notify_db_session, active_provider, inactive_provider)

        actual_provider = get_highest_priority_active_provider_by_notification_type(self.default_type)
        assert actual_provider == active_provider

    def test_gets_international(self, notify_db_session):
        ...
        commit_to_db(notify_db_session, international_provider, non_international_provider)
    ...
    def test_returns_none(self, notify_db_session):
        email_provider = self.provider_factory(notification_type=NotificationType.EMAIL)

        commit_to_db(notify_db_session, email_provider)
...
class TestGetActiveProvidersWithWeightsByNotificationType:
    ...
    def test_gets_weighted(self, notify_db_session):
        ...
        commit_to_db(notify_db_session, weighted_provider, unweighted_provider)
    ...
    def test_returns_empty_list(self, notify_db_session):
        email_provider = self.provider_factory(notification_type=NotificationType.EMAIL)
