...
@pytest.fixture
def service_sms_sender_no_message_service():
    class ServiceSmsSender:
        def __init__(self):
            self.sms_sender = "Test Sender"
...
def test_send_sms_callback_url(
    ...
    mocker.patch("app.clients.sms.twilio.Client", return_value=twilio_sms_client)

    # Mock the dao functions
    mocker.patch(
        "app.clients.sms.twilio.dao_get_service_sms_sender_by_service_id_and_number",
        return_value=service_sms_sender,
    )
    mocker.patch(
        "app.clients.sms.twilio.dao_get_service_sms_sender_by_id",
        return_value=service_sms_sender,
    )
    ...
    twilio_client_mock.messages.create.assert_called_once_with(
        to=to,
        messaging_service_sid=service_sms_sender.sms_sender_specifics[
...
def test_send_sms_callback_url(
    mocker, service_sms_sender_no_message_service, environment, expected_prefix
):
    to = "+1234567890"
    content = "Test message"
    reference = "test_reference"
    ...
    mocker.patch(
        "app.clients.sms.twilio.dao_get_service_sms_sender_by_service_id_and_number",
        return_value=service_sms_sender_no_message_service,
    )
    mocker.patch(
        "app.clients.sms.twilio.dao_get_service_sms_sender_by_id",
        return_value=service_sms_sender_no_message_service,
