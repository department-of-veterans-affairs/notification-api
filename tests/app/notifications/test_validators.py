...
def test_should_not_interact_with_cache_for_test_key(
    sample_service,
    mocker,
):
...
def test_check_service_message_limit_in_cache_over_message_limit_fails(
    ...
    with freeze_time("2016-01-01 12:00:00.000000"):
        ...
        service = sample_service(restricted=True, message_limit=4)
        with pytest.raises(TooManyRequestsError) as e:
            check_service_over_daily_message_limit(key_type, service)
        assert e.value.status_code == 429
        assert e.value.message == 'Exceeded send limits (4) for today'
...
def test_allows_api_calls_with_international_numbers_if_service_does_allow_int_sms(
    ...
    service = sample_service(
        ...
        service_permissions=SERVICE_PERMISSION_TYPES
    )
...
def test_check_service_email_reply_to_where_email_reply_to_is_found(
    ...
    service = sample_service()
    reply_to_address = sample_service_email_reply_to(service, email_address="test@test.com")
    assert check_service_email_reply_to_id(service.id, reply_to_address.id, EMAIL_TYPE) == "test@test.com"
...
def test_check_service_email_reply_to_id_where_service_id_is_not_found(
    ...
    reply_to_address = sample_service_email_reply_to(sample_service(), email_address="test@test.com")
    with pytest.raises(BadRequestError) as e:
    ...
    assert e.value.message == 'email_reply_to_id {} does not exist in database for service id {}' \
        .format(reply_to_address.id, fake_service_id)
...
def test_check_service_sms_sender_id_where_service_id_is_not_found(
    ...
    assert e.value.message == 'sms_sender_id {} does not exist in database for service id {}' \
        .format(sms_sender_id, fake_service_id)
...
def test_check_reply_to_email_type(
    ...
    service = sample_service()
    reply_to_address = sample_service_email_reply_to(service, email_address="test@test.com")
    assert check_reply_to(service.id, reply_to_address.id, EMAIL_TYPE) == 'test@test.com'
    ...
    def test_that_when_sms_sender_rate_exceed_rate_limit_request_fails(
        ...
        with freeze_time('2016-01-01 12:00:00.000000'):
            ...
            should_throttle.assert_called_once_with(
                sms_sender.sms_sender, service.rate_limit, 60
            )
            assert e.value.status_code == 429
