...
def test_process_sms_response_does_not_send_status_update_for_pending(
    ...
    send_mock = mocker.patch('app.celery.service_callback_tasks.send_delivery_status_to_service.apply_async')
    process_sms_client_response(status='2', provider_reference=str(sample_notification().id), client_name='firetext')
    send_mock.assert_not_called()
...
def test_process_sms_updates_sent_by_with_client_name_if_not_in_noti(
    ...
    success, error = process_sms_client_response(status='3', provider_reference=str(notification.id), client_name='MMG')
    assert error is None
...
def test_process_sms_does_not_update_sent_by_if_already_set(
    ...
    process_sms_client_response(status='3', provider_reference=str(notification.id), client_name='MMG')
