...
class TestLogin:
    ...
    def test_should_redirect_to_github_if_toggle_is_enabled(self, client):
        response = client.get('/auth/login')
    ...
    @pytest.mark.parametrize(
        'feature, idp', [(FeatureFlag.GITHUB_LOGIN_ENABLED, 'github'), (FeatureFlag.VA_SSO_ENABLED, 'va')]
    )
...
class TestRedeemToken:
    def test_should_return_401_if_cookie_verification_fails(self, client, mocker):
        mocker.patch('app.oauth.rest.verify_jwt_in_request', side_effect=NoAuthorizationError())
        response = client.get('/auth/redeem-token')

        assert response.status_code == 401

    def test_should_return_cookie_value_in_body(self, client, mocker, notify_api):
...
class TestLoginWithPassword:
    ...
    def test_login_with_password_succeeds_when_correct_credentials(self, notify_db_session, client, mocker):
        ...
        data = {'email_address': 'dummy@email.address', 'password': some_p}
    ...
    def test_should_return_401_when_wrong_password(self, client, mocker):
        ...
        response = client.post(
            '/auth/login',
            data=json.dumps({'email_address': 'some@email.address', 'password': 'incorrect_password'}),
    ...
    def test_should_return_401_when_user_not_found_by__email_address(self, client, mocker):
        ...
        response = client.post(
            '/auth/login',
            data=json.dumps({'email_address': 'some@email.address', 'password': 'silly-me'}),
            headers=[('Content-Type', 'application/json')],
        )
        assert response.status_code == 401
    ...
    def test_login_with_password_success_returns_token(self, notify_db_session, client, mocker):
        ...
        response = client.post(
            '/auth/login',
            data=json.dumps({'email_address': email_address, 'password': password}),
            headers=[('Content-Type', 'application/json')],
        )
