...
class TestTransformToFhirFormat:
    @pytest.mark.parametrize(
        'id_type, id_value, expected_fhir_format',
        [
...
class TestTransformFromFhirFormat:
    @pytest.mark.parametrize(
        'fhir_format_identifier, expected_identifier',
        [
    ...
    def test_should_transform_from_fhir_format(self, fhir_format_identifier, expected_identifier):
