...
class TestUpdateServiceCallback:
    def test_update_service_callback_updates_url(self, client, sample_service):
        service = sample_service()
        service_callback_api = create_service_callback_api(service=service, url='https://originalurl.com')
    ...
    def test_update_service_callback_updates_notification_statuses(self, client, sample_service):
        service = sample_service()
        service_callback_api = create_service_callback_api(service=service, notification_statuses=['cancelled'])
        data = {
    ...
    def test_update_service_callback_raises_400_when_wrong_request(self, client, sample_service, request_data):
        service = sample_service()
        service_callback_api = create_service_callback_api(service=service, notification_statuses=['technical-failure'])
