...
def test_user_service_role_serializes_with_updated_at(client, sample_user_service_role_udpated_at, sample_user, sample_service):
    res = sample_user_service_role_udpated_at.serialize()
    assert res['id'] is not None
    assert res['role'] == "admin"
    assert res['user_id'] == str(sample_user.id)
    assert res['service_id'] == str(sample_service.id)
    assert res['updated_at'] == sample_user_service_role_udpated_at.updated_at.isoformat()
